...
class Trie
  ...
  class Node
    def initialize(word, values, descendants = {})
      @word = word
      @values = values
    ...
    def add(word, value)
      ...
      common_prefix = CommonPrefix.new(@word, word).max
      raise ArgumentError, "Node '#{@word}' has no common prefix with word '#{word}'" if common_prefix.blank?

      # Case A: same words -> @word:hello - word:hello
      if common_prefix.length == @word.length && common_prefix.length == word.length
        return Node.new @word, @values.push(value), @descendants
      end

      suffix_a = @word.suffix(common_prefix)
      suffix_b = word.suffix(common_prefix)
      # Case B: suffix of @word is empty -> @word:hell - word:hello
      if suffix_a.empty?
        key = suffix_b[0]
    ...
    def values(word, match = :starts_with)
      Enumerator.new do |yld|
        if @word == word || match == :starts_with && @word.start_with?(word)
          ...
          suffix = word.suffix(CommonPrefix.new(@word, word).max)
          key = suffix[0]
